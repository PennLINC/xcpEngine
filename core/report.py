...
for i in modules1:
    if i == 'struc':
        ...
        'size': 10}
         ...
         sns.set(style="white", palette="bright", color_codes=True,font_scale=2)
         plot_stat_map(stat_map_img=statmapalff,bg_img=bgimg,cut_coords=(0,0,0),draw_cross=False,vmax=2,symmetric_cbar=True,colorbar=True,axes=ax1,black_bg=False)
         ...
         ax2 = fig.add_subplot(gs1[-1, -1])
         sns.set(style="white", palette="bright", color_codes=True,font_scale=2)
         plot_stat_map(stat_map_img=statmapreho,bg_img=bgimg,cut_coords=(0,0,0),draw_cross=False,vmax=2,symmetric_cbar=True,colorbar=True,axes=ax1,black_bg=False)
         sns.distplot(dat1,kde=False,ax=ax2,color='b')
