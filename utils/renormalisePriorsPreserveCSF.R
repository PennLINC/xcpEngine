...
###################################################################]
writeImages <- function(images, outputRoot, templateImage, out, mask) {
...
priorImages[[1]]        <- readNifti(csfpath)
out                     <- priorImages[[1]]
for (i in 2:(length(impaths)+1)) {
   priorImages[[i]]     <- readNifti(impaths[i-1])
}
...
###################################################################
writeImages(priors, outputRoot, csfpath, out, mask)
